...
export default function ProjectSidebar() {
  ...
  const routeProjectName = params?.project_name ? decodeURIComponent(params.project_name) : "";
  ...
  useEffect(() => {
    ...
    const match = projects.find((p: Project) => p.name === routeProjectName);
  ...
  const handleSelectProject = (project: Project | null, close?: () => void) => {
    ...
    router.push(`/${encodeURIComponent(username)}/${encodeURIComponent(project.name)}/`);
  };
  ...
  <aside className="w-72 shrink-0 self-stretch border-r-2 border-black bg-slate-900/80 text-slate-200">
    <div className="h-full p-4 flex flex-col gap-6">
      ...
      <DropDown
        label={
          <div className="min-w-0">
            ...
            <div className="truncate font-medium">{currentProject ? currentProject.name : "None"}</div>
          </div>
        }
        align="left"
        disabled={!loggedIn}
              ...
              <div className="flex flex-col gap-1">
                <button
                  ...
                  className={`group flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-sm transition-colors cursor-pointer hover:bg-slate-800/70 ${
                    !currentProject ? "bg-purple-600/15 text-slate-100" : "text-slate-200"
                  }`}
                  onClick={() => handleSelectProject(null, close)}
                >
                  <span className="truncate">None</span>
                  <span
                    className={`shrink-0 ${
                      !currentProject ? "text-purple-300" : "text-slate-500 opacity-0 group-hover:opacity-100"
                    }`}
                    aria-hidden="true"
                  >
                    <IconCheck size={16} />
                ...
                {isLoading && <div className="px-3 py-2 text-sm text-slate-400">Loading projects…</div>}
                {!isLoading && error && (
                  <div className="px-3 py-2 text-sm text-red-300">Failed to load projects.</div>
                )}
                ...
                {!isLoading &&
                  ...
                  projects?.map((project: Project) => {
                    const selected = project.puid === currentProject?.puid;
                    return (
                      <button
                        type="button"
                        key={project.puid}
                        className={`group flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-sm transition-colors cursor-pointer hover:bg-slate-800/70 ${
                          selected ? "bg-purple-600/15 text-slate-100" : "text-slate-200"
                        }`}
                        onClick={() => handleSelectProject(project, close)}
                      >
                        <span className="truncate">{project.name}</span>
                        <span
                          className={`shrink-0 ${
                            selected ? "text-purple-300" : "text-slate-500 opacity-0 group-hover:opacity-100"
                          }`}
                          aria-hidden="true"
                        >
                          <IconCheck size={16} />
      ...
      >
        ...
        <div className="font-medium">{currentProject ? "Project Dashboard" : "Select a project"}</div>
      </Link>

      {/* Navigation Links */}
      <nav className="flex flex-col gap-1">
        <div className="px-2 pb-2 text-xs font-medium uppercase tracking-wider text-slate-400">Components</div>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={18} className={active ? "text-purple-300" : "text-slate-400"} />
              <span>{item.name}</span>
            </Link>
          );
        })}
      ...
      <div className="mt-auto pt-4 border-t border-slate-800">
        {!loggedIn && <div className="text-xs text-slate-500">Log in to view projects.</div>}
      </div>
    </div>
  </aside>
  );
