				...
				{(title || showCloseButton) && (
					<div className="flex items-start justify-between gap-4 border-b border-slate-800 px-4 py-3">
						<div className="min-w-0">
							{title && (
								<div id={titleId} className="truncate text-base font-semibold text-slate-100">
									{title}
								</div>
							)}
							{description && (
								<div id={descriptionId} className="mt-0.5 text-sm text-slate-400">
									{description}
								</div>
							)}
						</div>
						...
						{showCloseButton && (
							<button
								type="button"
								onClick={close}
								className="shrink-0 rounded-lg border border-slate-700 bg-slate-900/60 p-2 text-slate-300 transition-colors hover:border-purple-500/60 hover:bg-slate-800/60 hover:text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500/40"
								aria-label="Close"
							>
								<IconX size={16} />
							</button>
